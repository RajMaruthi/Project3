...
/********************************
	...
	* ******************************/
	 ...
	 * Purpose : this will create the customer
	 * Author : Maruthiraja BN
	 * Reviewer : 
	...
	public boolean create_Customer(WebDriver obrowser,String CustName,String Custdescription)
	{
		String strStatus=null;
		boolean bolstatus=false;
		try
		{
			strStatus+=appInd.clickObject(obrowser, "Click_Task");
			strStatus+=appInd.clickObject(obrowser, "Click_AddNew");
			
			List<WebElement>list=appInd.createAndGetObjectList(obrowser, "Select_List");
			for(int i=0;i<list.size();i++)
			{
				if(list.get(i).getText().equals("+ New Customer"))
					list.get(i).click();
				break;		
			}
			
			strStatus+=appInd.setObject(obrowser, "CustomerName", CustName);
			strStatus+=appInd.setObject(obrowser, "Description_Customer", Custdescription);
			strStatus+=appInd.clickObject(obrowser, "Create_Customer");
		...
		{
			appInd.writeLog("Exception", "Exception while executing the 'create_Customer'" +e.getMessage());
			bolstatus=false;
		}
		finally
		{
	...
	/********************************
	 * Method Name : handleRadioButton
	 * Purpose : this will handle the radio button
	 * Author : Maruthiraja BN
	 * Reviewer : 
	 * Date of Creation : 10-June-2020 
	 * Date of modification :
	...
	public boolean handleRadioButton(WebDriver oDriver, String Marital_Status)
	{
		
		List<WebElement> RadioItemList=null;
		String strMaritalStatus=null;
		Boolean strStatus=false;
		
		try
		{
			RadioItemList=appInd.createAndGetObjectList(oDriver, "Merital_Status_Radio_Btn");
		    for(int i=0;i<RadioItemList.size();i++)
			{
				strMaritalStatus=RadioItemList.get(i).getText();
				if(strMaritalStatus.equalsIgnoreCase(Marital_Status))
				{
					RadioItemList.get(i).click();
					appInd.writeLog("Pass", "It's selected radio button "+Marital_Status+"");
					strStatus=true;
					break;
				}
			}
		...
		{
			appInd.writeLog("Exception", "Exception while executing the 'handleRadioButton' method" +e.getMessage());
			strStatus=false;
			return strStatus;
		}
	}
	...
	public boolean handleDropdown( WebDriver obDriver,String strObject,String selectItem,String SelectByItem ) 
	{
		...
		try
		{
			oEle=appInd.createAndGetObject(obDriver, strObject);
			
			if(oEle!=null)
			{
				Select selectObj=new Select(oEle);
				
				selectObj.selectByValue(selectItem);
				strStatus=true;
				
				if(onlyDigits(selectItem))
				{
					int Index=Integer.parseInt(selectItem);
				...
				else
				{
					switch(SelectByItem)
					{
						...
						case "selectByVisibleText":
							selectObj.selectByVisibleText(selectItem);
							strStatus=true;
							break;
							
						default:
							System.out.println("Invalid input for selecting dropdown Items");
					}			
				}			
			}
			else
			{
				appInd.writeLog("warning", "Webelement is null while executing the 'handledropdown' method ");strStatus=false;
				strStatus=false;
				return strStatus;	
			}
		...
		{
			appInd.writeLog("Exception", "Exception while executing the 'handledropdown' method"+e.getMessage());
			strStatus=false;
			return strStatus;
		}
